...
alter table comment add constraint FK_说说评论 foreign key (say_id)
...
alter table reply add constraint FK_回复 foreign key (comment_id)
...
alter table say_picture add constraint FK_属于 foreign key (ptype_id)
      references picture_type (ptype_id) on delete restrict on update restrict;
